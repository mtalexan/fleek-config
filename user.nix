...
in
{
  ...
  # matching config.home.file.X.enable with the option value.
  # Because you can't mix the config.home. and config = { home.Y = ...} formats in the same file,
  # we have to group it all under a single big config={} instead.  The options.custom.files therefore 
  # all have to go together.

  options = {
    custom.files = {
      ".config/distrobox/distrobox.conf".enable = lib.mkEnableOption(lib.mdDoc "distrobox.conf file auto-population");
      ".config/distrobox".enable = lib.mkEnableOption(lib.mdDoc "distrobox pre-init and init hooks");
      ".config/containers".enable = lib.mkEnableOption(lib.mdDoc "podman ubuntu-like registry and storage settings");
      ".config/containers/registries.conf.d/000-shortnames.conf".enable = lib.mkEnableOption(lib.mdDoc "podman predefined list of public image aliases");
    };
  };
